...
public class VoiceServiceImpl implements VoiceService {
    ...
    public String queryCommand(String commandType, String area) {
        ...
        if (commandType.equals("01")) {
            ...
            String hz = jcInfoService.getHZByArea(area.substring(0, 2));
            if (hz.equals("")) {
                message = "查询失败";
                msg = url + "没有查到相关信息";
            } else {
                msg = url + area + "的振荡频率是" + hz;
            }
        } else if (commandType.equals("08")) {
            String percent = jcInfoService.getZNByArea(area.substring(0, 2));
            if (percent.equals("")) {
                message = "查询失败";
                msg = url + "没有查到相关信息";
            } else {
                msg = url + area + "的阻尼比是" + percent;
                message = "查询成功";
            }
        }
    ...
    public String operate(String commandType, String parameter) {
        ...
        if (commandType.equals("00")) {
            if (parameter.equals("一百一十千伏")) {
                voice_map.put("type", "1");
            } else if (parameter.equals("二百二十千伏")) {
                voice_map.put("type", "2");
            } else if (parameter.equals("五百千伏")) {
                voice_map.put("type", "3");
            } else if (parameter.equals("一千千伏")) {
    ...
    @Override
    public void getOrder(String node_id, String operation) {
        Map<String, String> voice_map = new HashMap<>();
        String url = "http://192.168.101.80:8100/tts?access_token=speech&language=zh&domain=1&voice_name=静静&text=";
        String quality = "";
        ...
        String message = "请注意！！，虚拟沙盘系统" + quality + "节点" + node_id + "的电压幅值";
        voice_map.put("type", "sp");
        voice_map.put("message", url + message);
        voice_map.put("alter", message);
        voiceSocket.sendMessage(JSONObject.fromObject(voice_map).toString());
    }
}
